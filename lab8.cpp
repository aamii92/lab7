#include <iostream>
    ...
    public:
        ...
        float Perimetr() override { return 2 * 3.14 * r; }
    };
    ...
    public:
        ...
        float Perimetr() override { return 4 * r; }
    };
    ...
    // Square классын параметртэй байгуулагч — TwoDShape-ийг дуудаж байна
        ...
        float Perimetr() override { return 3 * r; }
    };
    ...
    // Периметрээр нь эрэмбэлэх ( bubble sort)
    for (int i=0; i<6; i++) {
        for (int j=i+1; j<6; j++) {
            if (shapes[i]->Perimetr() > shapes[j]->Perimetr()) {
                swap(shapes[i], shapes[j]);
            }
        }
    }
    cout << "\n--- Периметрээр эрэмбэлсэн жагсаалт \n";
    for (int i = 0; i < 6; i++) {
